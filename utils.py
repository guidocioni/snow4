...
folder_images = "/scratch/local1/m300382/snow4/"
...
def get_snow_data(datetime_now):
    ...
    filename='sn_vzp%s_obdn%s.asc.zip'% (datetime_now.strftime('%Y%m%d')+run,datetime_now.strftime('%y%m%d')+run)
    ...
    time=pd.date_range(end=datetime.strptime(date_end_file,'%y%m%d%H'), freq='1h', periods=6)
...
def get_coords(height=False):
    ...
    height values.'''
    coord=pd.read_fwf(grid_file, skiprows=20, header=None, lineterminator='\n', compression='zip')
    longitudes=np.array(coord)[0:909,1:].astype("float")

    coord=pd.read_fwf(grid_file, skiprows=931, header=None, lineterminator='\n', compression='zip')
    latitudes=np.array(coord)[0:909,1:].astype("float")
        
    if height:
        coord=pd.read_fwf(grid_file, skiprows=1842, header=None, lineterminator='\n', compression='zip')
